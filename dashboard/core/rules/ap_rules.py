...
def get_ap_checks():
    ...
    checks = [
        ...
        ('SUP_NAME_DUP', 10, 'Suppliers', 'Uniqueness', 'Medium',
         ...
         lambda df: df.duplicated(subset=['house', 'client', 'apar_name', 'address', 'zip_code'], keep=False) & (df['apar_name'].str.strip().str.len() > 1)),
        ...
        ('SUP_NAME_DUP_ANY', 10, 'Suppliers', 'Uniqueness', 'Low',
         ...
         lambda df: df.duplicated(subset=['house', 'client', 'apar_name'], keep=False) & (df['apar_name'].str.strip().str.len() > 1)),
        ...
        ('SUP_VAT_DUP', 10, 'Suppliers', 'Uniqueness', 'High',
         ...
         lambda df: df.duplicated(subset=['house', 'client', 'vat_reg_no'], keep=False) & df['vat_reg_no'].notna() & (df['vat_reg_no'].str.strip().str.len() > 1)),
        ...
        ('SUP_BANK_SORT_DUP', 10, 'Suppliers', 'Uniqueness', 'High',
         ...
         lambda df: df.duplicated(subset=['house', 'client', 'bank_account', 'clearing_code'], keep=False) & df['bank_account'].notna() & df['clearing_code'].notna() & (df['bank_account'].str.strip().str.len() > 1) & (df['clearing_code'].str.strip().str.len() > 1)),
        ...
        ('SUP_BANK_DUP', 10, 'Suppliers', 'Uniqueness', 'High',
         ...
         lambda df: df.duplicated(subset=['house', 'client', 'bank_account', 'clearing_code', 'vat_reg_no'], keep=False) & df['bank_account'].notna() & df['clearing_code'].notna() & df['vat_reg_no'].notna() & (df['bank_account'].str.strip().str.len() > 1) & (df['clearing_code'].str.strip().str.len() > 1) & (df['vat_reg_no'].str.strip().str.len() > 1)),
...
('SUP_DORMANT', 10, 'Suppliers', 'Timeliness', 'Medium',
         ...
         lambda df, frames: (
             ...
             ~((df['house'] == 'HOL') & df['apar_id'].astype(str).str.strip().str.startswith('3'))
         )),
