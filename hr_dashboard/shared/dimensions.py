...
import numpy as np
import textwrap
from hr_dashboard.shared.ui import card, section_header, RAG_HEX, HOUSE_HEX, CHART_LAYOUT
from hr_dashboard.core.theme import DISPLAY_FONT

DIMENSION_DESCRIPTIONS = {
    'Completeness': 'Ensuring all required data fields are populated. Missing values in critical fields like Bank Details or Tax Codes will cause payslips to fail processing.',
    'Validity': 'Ensuring data follows the correct format and business rules. For example, National Insurance numbers must follow the standard HMRC pattern.',
    'Uniqueness': 'Identifying duplicate records. Duplicate employees or payroll transactions lead to double payments and inaccurate headcount reporting.',
    'Timeliness': 'Assessing how up-to-date records are, particularly payroll transactions stuck in a pending state that need resolving before the next pay run.',
    'Referential Integrity': 'Ensuring consistency between related tables. For example, every payroll transaction must belong to a valid, active employee record.',
    'Consistency': 'Ensuring data is consistent across different fields, tables, or systems — for example, payroll bank details matching the employee master.',
}
...
def render_dimension_widget(dim_name, dq_results, key_prefix=''):
    ...
    for house in houses:
        ...
        fig.add_trace(go.Bar(
            name=house, x=rates, y=check_descs, orientation='h', marker_color=HOUSE_HEX.get(house, '#CC092F'),
            offsetgroup=house, customdata=np.stack((check_ids, [house] * len(check_ids)), axis=-1),
            hovertemplate=f'<b>{house}</b><br>%{{y}}<br>Error Rate: %{{x}}%<extra></extra>',
            text=labels, textposition=pos, textfont=dict(size=10, color=col, family='Inter')
        ))
    ...
    def _house_score(error, house):
        if error is None:
            return html.Div()
        house_color = HOUSE_HEX.get(house, '#CC092F')
    ...
    summary_content = html.Div(style={
        ...
        html.Div(style={'display': 'flex', 'alignItems': 'center', 'gap': '12px'}, children=[
            ...
            html.Div([
                ...
                html.Div(f"{total_checks} checks", style={
    ...
    return card([
        ...
        html.Div([
            dash_table.DataTable(
                ...
                style_table={'overflowX': 'auto'}, style_cell={'textAlign': 'left', 'padding': '12px', 'fontFamily': 'Inter', 'fontSize': '12px'},
                style_header={'backgroundColor': '#F8FAFC', 'fontWeight': 'bold', 'color': '#64748B', 'borderBottom': '2px solid #E2E8F0'},
                style_data_conditional=[
                    {'if': {'column_id': 'Action'}, 'color': '#CC092F', 'fontWeight': '700', 'cursor': 'pointer', 'textDecoration': 'underline', 'backgroundColor': '#FBE7EA'},
                    ...
                    {'if': {'column_id': 'RAG', 'filter_query': '{RAG} eq "Green"'}, 'color': '#1a7a4a', 'fontWeight': 'bold'}
                ],
            ),
        ])
    ])
