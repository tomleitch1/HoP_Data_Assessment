...
-- Output:    customer_master_HOL.csv
-- Server:    mdata837
-- NOTE: attribute_id for customer records in agladdress is NOT yet confirmed.
--       Before running, verify with:
--       SELECT DISTINCT attribute_id FROM agladdress WHERE client = 'LA' AND address_type = '1'
--       Compare against the supplier attribute_id (A5 for HoC) to identify the customer equivalent.
