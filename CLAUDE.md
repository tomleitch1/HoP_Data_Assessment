...
- **HOL has no equivalent Commitments extract worth using** (confirmed by the user directly, August 2026) — `contract_total_commitments.csv` resolves almost exclusively to HOC in real data (see the earlier finding above: 4,143 of 4,148). HOL masters its contract reference completely differently: it lives in **`gl_dimension_values_HOL.csv`** (`agldimvalue`), specifically the row where **`dim_position == '5'`** and **`dim_description == 'Contract Number'`** — `dim_value` on that row *is* the contract reference, joining directly to `atamis_contracts.contract_ref` the same way `unit4_commitments.u4_contract_id` does for HOC. See `_build_unit4_contract_refs()` in `data_engine.py` below.

### UNIT4_COMMIT_NOT_IN_CONTRACTS — two different sources per house
This check ("does this Unit4-side contract reference trace back to a real Atamis contract?") is the one check in this domain whose *population itself* is sourced completely differently per house, not just filtered differently. Rather than teaching every other Commitments check (`REMAINING_MISMATCH`, `DATE_INVALID`, `DUP_ID`, `OVERSPEND`, `SUPPLIER_ORPHAN`) about a second source it doesn't have data for, `_build_unit4_contract_refs(frames)` in `data_engine.py` builds a dedicated frame, `unit4_contract_refs`, used only by this one check:
- A full copy of `unit4_commitments` (same columns — so this check's existing lambda and modal enrichment work unchanged for HOC, with real `contract_title`/`supplier_name`/etc.), tagged `_source = 'unit4_commitments'`.
...
- **`UNIT4_COMMIT_NOT_IN_CONTRACTS` given a second, HOL-specific source (August 2026, per direct request)** — HOL has no usable Commitments extract of its own (confirmed by the user: real data resolves that file almost entirely to HOC), so its contract reference is instead sourced from `gl_dimension_values_HOL.csv`'s `dim_position == '5'` (`dim_description == 'Contract Number'`) rows. See "UNIT4_COMMIT_NOT_IN_CONTRACTS — two different sources per house" above for the full design (`_build_unit4_contract_refs()`, the `unit4_contract_refs` frame, cache-signature and tab-scoped-loading consequences). Verified via a forced synthetic case (both HOC- and HOL-sourced rows resolve and flag correctly); shows HOC-only on dummy data since dummy HOL's `dim_position == '5'` attribute is still labelled `'Counterpart'`, a known generator gap.
