...
**Verdict logic (`_reliability_verdict`):** for each contract's `contract_ref`, look up every distinct `house` value it resolves to across `unit4_contract_refs` (which already carries both signals — a commitment's own supplier-derived house, and a flat `'HOL'` tag for a GL Contract Number match):
