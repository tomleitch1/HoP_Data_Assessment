...
- 26 checks defined in `dashboard/core/rules/po_rules.py` (scope 15) — `PO_TERMINATED_WITH_INVOICING` and `PO_LINE_NEVER_MATCHED` were removed (August 2026) per direct request, dropping the count from 28. 23 currently appear in `dq_results` against dummy data; the 3-tier unmatched-receipt-aging checks below have an empty shared population on dummy data and so don't appear at all (see the "shared base population is empty" note above), which is expected engine behaviour, not a bug. Includes 4 cross-domain checks against Suppliers (`asuheader`)/GL (`aglaccounts`), 5 added after direct pushback on real-data behaviour (`PO_DUP_HEADER`, `PO_FUTURE_ORDER_DATE`, `PO_HDR_LINE_DATE_MISMATCH`, `PO_ARR_EXCEEDS_AMOUNT`, `PO_LINE_NO_CATEGORY`), 3 from the user's own manual Excel exploration of real PO lines (`PO_LINE_MATCH_EXCEEDS_RECEIPT`, `PO_LINE_INVOICED_AHEAD_OF_RECEIPT`, `PO_LINE_AMENDED_VALUE_MISMATCH`), 2 calculation-integrity checks (`PO_LINE_VOW_CALC_MISMATCH`, `PO_LINE_ARR_CALC_MISMATCH`), and a 3-tier unmatched-receipt-aging check that replaced the original single-tier `PO_LINE_STALE_UNRESOLVED` (`PO_LINE_UNINVOICED_RECEIPT_3TO6M`, `_6TO12M`, `_OVER12M`) — see PO Domain section above for the full list
