...
**DQ checks implemented in `po_rules.py` (19 total):**
...
| `PO_TERMINATED_WITH_INVOICING` | apodetail | Consistency | Medium | status == T | `GREATEST(arr_amount, invoiced)` materially non-zero — see below for why this isn't "any activity" |
...
| `PO_HDR_LINE_DATE_MISMATCH` | apodetail (joins apoheader) | Consistency | Low | all rows | line's own `order_date` != its header's `order_date` — same "not yet confirmed" caveat as the status mismatch check |
...
| `PO_LINE_NEVER_MATCHED` | apodetail | Consistency | High | status == F | Finished line with real activity (`vow_amount` or `invoiced` non-zero) but `arr_amount` and `arr_val` both flat zero — matching never ran against this line at all |
...
| `PO_LINE_UNINVOICED_RECEIPT_3TO6M` | apodetail | Consistency | Low | shared base pop. (see below) | 90 ≤ days since `deliv_date` < 180 |
| `PO_LINE_UNINVOICED_RECEIPT_6TO12M` | apodetail | Consistency | Medium | shared base pop. (see below) | 180 ≤ days since `deliv_date` < 365 |
| `PO_LINE_UNINVOICED_RECEIPT_OVER12M` | apodetail | Consistency | High | shared base pop. (see below) | days since `deliv_date` ≥ 365 |
...
| `PO_LINE_ARR_CALC_MISMATCH` | apodetail | Validity | Medium | all rows | `arr_amount` ≠ `arr_val * unit_price` — same corruption check on the matched side |

All join checks use named helper functions in `po_rules.py` rather than inline lambdas (`_po_finished_with_balance`, `_po_line_status_mismatch`, `_po_hdr_line_date_mismatch`, `_po_orphaned_supplier`, `_po_inactive_supplier`, `_po_line_orphan_account`, `_po_line_closed_account`). `_po_line_never_matched` and `_po_line_stale_unresolved` are named single-argument helpers (no join, but multi-condition logic too complex for a one-line lambda). The first two mirror the exact vectorized pandas logic already proven in `po.py`'s `_compute_metrics` (`finished_bal` and `mismatch_mask` respectively). Population filtering is a per-check_id branch in both `run_dq_analysis()` and `get_failing_records()` in `data_engine.py` (same dual-location pattern as every other domain's per-check population overrides): for `apoheader`, default excludes `T` (raised in error), with `PO_DUP_HEADER` using the full population regardless of status, `PO_STUCK_NOT_ORDERED` scoped to `N` only, `PO_FINISHED_WITH_BALANCE` scoped to `F` only, and `PO_INACTIVE_SUPPLIER` scoped to `O`/`N`/`A` only; for `apodetail`, default is all rows, with `PO_TERMINATED_WITH_INVOICING` scoped to `T` only.

**Why `PO_TERMINATED_WITH_INVOICING` checks "materially non-zero," not "any activity" (added after direct pushback on the first version):** the original idea was "T = raised in error, so it shouldn't have real activity" — but real T-status line inspection showed the opposite is the *normal* case: most T lines have negligible/zero `amount` and `invoiced` (noise), but a meaningful number show partial or fully-matching `invoiced` values against their `amount` — i.e. genuine activity was processed before the PO was terminated, which apparently happens routinely rather than being an anomaly. A blanket "any activity" check would have fired on close to 100% of terminated lines, the same failure mode as the original `vow_amount`-based version of `PO_FINISHED_WITH_BALANCE`. The check as built uses `GREATEST(arr_amount, invoiced) > 0.01` — deliberately excludes trivial/negative noise (e.g. a `-0.01` residual) while still catching genuine partial or full invoicing on a since-terminated PO.
...
**Known dummy-data limitations (the July 2026 batch of 6 checks):** several show 0% against dummy data purely because the generator never introduces the variance being checked for — verified as generator gaps, not logic bugs, by forcing synthetic mismatches and confirming each check fires correctly (see below). `PO_DUP_HEADER` (never generates a duplicate `order_id`), `PO_FUTURE_ORDER_DATE` (no future dates modeled), `PO_HDR_LINE_DATE_MISMATCH` (every line's `order_date` is copied straight from its header), `PO_LINE_NO_CATEGORY` (`art_gr_id` is always assigned), `PO_ARR_EXCEEDS_AMOUNT` (invoiced/arr figures are always generated as a fraction of `amount`, never above it), and `PO_TERMINATED_WITH_INVOICING` (T-status lines get `arr_pct = vow_pct = 0.0` unconditionally in the generator, so `invoiced` is always exactly zero on dummy T lines — unlike the real data this check was built for).
...
**Five more checks added (July 2026) from direct Excel exploration of real PO lines**, distinct from the "further checks" round above — this batch came from the user manually filtering the raw extract in Excel and finding a concrete example (a Finished line, delivered in 2015, fully received, `arr_amount`/`arr_val` = 0, `invoiced` = 32.48 against an amount of 37.5) that the existing header-level `PO_FINISHED_WITH_BALANCE` check could wash out inside its PO-level 5%-of-total aggregate if the rest of that PO reconciled normally. `PO_LINE_NEVER_MATCHED` isolates that exact shape at line level: real activity recorded, but the matching-specific fields never touched at all — a different root cause (matching bypassed entirely) from a partially-reconciled PO. `PO_LINE_MATCH_EXCEEDS_RECEIPT` also came directly out of that same Excel session — the user's own example had `vow_val = 5` computed as `amount / unit_price` (37.5 / 7.5), which contradicts the SQL extract's comment describing `vow_val`/`arr_val` as "value in order currency" rather than quantity; logged as a discrepancy worth asking Parliament about, but the check logic (`arr_val` should never exceed `vow_val`) holds regardless of which interpretation is correct.
...
**`PO_LINE_STALE_UNRESOLVED` (30-day, single-tier) was replaced with a three-tier aging check (August 2026)** after the user judged it told too little of the story: a line 30 days past delivery can easily still be a normal in-flight receipt awaiting invoice, not a defect. Replaced with a shared base population — "unmatched open receipt" (`status NOT IN ('F','C','T')`, `vow_amount > 0`, `GREATEST(arr_amount, invoiced) = 0` — not `invoiced` alone, per the same field-ambiguity reasoning as `PO_FINISHED_WITH_BALANCE`, see `QUESTIONS_FOR_PARLIAMENT.md` #5) — sliced into three mutually exclusive age tiers by `deliv_date`, each carrying the severity the user assigned to its own narrative role: `PO_LINE_UNINVOICED_RECEIPT_3TO6M` (Low — "routine monitoring, not urgent"), `PO_LINE_UNINVOICED_RECEIPT_6TO12M` (Medium — "should be getting chased operationally"), `PO_LINE_UNINVOICED_RECEIPT_OVER12M` (High — "migration-blocking, needs a decision before cutover"). The shared population logic lives in `_po_unmatched_receipt_population()` in `data_engine.py`; each tier's own filter is a parametrized closure, `_po_unmatched_receipt_age_tier(min_days, max_days=None)` in `po_rules.py`, so the three tuples don't duplicate the age-window arithmetic. A `days_since_delivery` column is computed in `get_failing_records()` (not an early return — it's added to the row and falls through the generic tail) so the evidence table shows the exact age driving each row's tier, not just its `deliv_date`.

**All three tiers show 0% / are entirely absent from `dq_results` on dummy data** — the shared base population itself is empty (0 rows before any age split). Traced directly: of the 195 dummy lines that are genuinely received (`vow_amount > 0`) and still open, **all 195** already carry a non-zero `arr_amount`/`invoiced` (min £2.34) — `generate_po_dummy_data.py` sets `invoiced = arr_amount` exactly and never leaves both at zero once a line is received, the same known generator gap already documented for `PO_FINISHED_WITH_BALANCE`. Since `total == 0`, `run_dq_analysis` skips these three checks from `dq_results` entirely (same as any check with zero population) — they won't appear on the PO tab's scorecard/grid at all against dummy data, which is expected engine behaviour, not a bug. Verified the logic itself is correct via a forced synthetic case: 3 synthetic rows aged 120/270/400 days landed in tiers 1/2/3 exactly as expected, and a 4th (Finished status) and 5th (`vow_amount = 0`) were correctly excluded from the base population.

**Two calculation-integrity checks added (August 2026), completing a test the user had asked for earlier that was missed the first time round.** `PO_LINE_VOW_CALC_MISMATCH` and `PO_LINE_ARR_CALC_MISMATCH` test whether `vow_amount`/`arr_amount` equal their corresponding `vow_val`/`arr_val` times `unit_price` — a third variant of this test (`invoiced <> value_1 * unit_price`) cannot be built at all, since `value_1` is not a column extracted in `po_detail_HOC_run.sql`. Both show ~92.8% failing on dummy data — confirmed via a forced synthetic case that the logic itself is correct, and confirmed the cause is `generate_po_dummy_data.py` building `vow_val`/`arr_val` from an exchange rate (`vow_amount * exch`), not from `vow_amount / unit_price` — a generator artifact, not a bug. These checks are actually the most direct way to settle the open `vow_val`/`arr_val` quantity-vs-value question logged in `QUESTIONS_FOR_PARLIAMENT.md` #5: if `vow_amount = vow_val * unit_price` holds on real data, that confirms `vow_val` is a received quantity (consistent with the user's own real example, 5 × 7.5 = 37.5), not "value in order currency" as the SQL extract's column comment states.
...
- 28 checks defined in `dashboard/core/rules/po_rules.py` (scope 15) — 25 currently appear in `dq_results` against dummy data; the 3-tier unmatched-receipt-aging checks below have an empty shared population on dummy data and so don't appear at all (see the "shared base population is empty" note above), which is expected engine behaviour, not a bug. Includes 4 cross-domain checks against Suppliers (`asuheader`)/GL (`aglaccounts`), 6 added after direct pushback on real-data behaviour (`PO_DUP_HEADER`, `PO_FUTURE_ORDER_DATE`, `PO_HDR_LINE_DATE_MISMATCH`, `PO_TERMINATED_WITH_INVOICING`, `PO_ARR_EXCEEDS_AMOUNT`, `PO_LINE_NO_CATEGORY`), 4 from the user's own manual Excel exploration of real PO lines (`PO_LINE_NEVER_MATCHED`, `PO_LINE_MATCH_EXCEEDS_RECEIPT`, `PO_LINE_INVOICED_AHEAD_OF_RECEIPT`, `PO_LINE_AMENDED_VALUE_MISMATCH`), 2 calculation-integrity checks (`PO_LINE_VOW_CALC_MISMATCH`, `PO_LINE_ARR_CALC_MISMATCH`), and a 3-tier unmatched-receipt-aging check that replaced the original single-tier `PO_LINE_STALE_UNRESOLVED` (`PO_LINE_UNINVOICED_RECEIPT_3TO6M`, `_6TO12M`, `_OVER12M`) — see PO Domain section above for the full list
