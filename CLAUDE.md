...
| `PO_LINE_STALE_UNRESOLVED` | apodetail | Consistency | Medium | status NOT IN (T,C,F) | `deliv_date` more than 30 days old — delivered but stalled before progressing to invoice or close |
...
**`PO_LINE_STALE_UNRESOLVED` shows an inflated 85.5% against dummy data** because `generate_po_dummy_data.py` assigns `deliv_date` and `status` independently of each other — dummy "still open" lines are scattered randomly across delivery dates back to 2009, rather than skewing toward recently-delivered (as a realistically-managed live PO book would). Generator unrealism, not a check logic bug — real data should show a much smaller, genuinely-stale tail. 30-day threshold chosen directly by the user over 90/180-day alternatives when this batch was scoped.
...
- 26 checks in `dashboard/core/rules/po_rules.py` (scope 15), including 4 cross-domain checks against Suppliers (`asuheader`)/GL (`aglaccounts`), 6 added after direct pushback on real-data behaviour (`PO_DUP_HEADER`, `PO_FUTURE_ORDER_DATE`, `PO_HDR_LINE_DATE_MISMATCH`, `PO_TERMINATED_WITH_INVOICING`, `PO_ARR_EXCEEDS_AMOUNT`, `PO_LINE_NO_CATEGORY`), 5 from the user's own manual Excel exploration of real PO lines (`PO_LINE_NEVER_MATCHED`, `PO_LINE_MATCH_EXCEEDS_RECEIPT`, `PO_LINE_INVOICED_AHEAD_OF_RECEIPT`, `PO_LINE_AMENDED_VALUE_MISMATCH`, `PO_LINE_STALE_UNRESOLVED`), and 2 calculation-integrity checks (`PO_LINE_VOW_CALC_MISMATCH`, `PO_LINE_ARR_CALC_MISMATCH`) — see PO Domain section above for the full list
