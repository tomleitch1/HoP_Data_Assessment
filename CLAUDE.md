...
- `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS` / `ATAMIS_COMMIT_NOT_IN_CONTRACTS` — contract existence in each direction, via the confirmed Contract Reference / Contract Id join (see The join keys, above). The reverse direction (a Unit4 commitment with no Atamis contract at all) is more surprising, so it's High rather than Medium severity.
