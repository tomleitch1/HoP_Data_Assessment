...
- 19 checks defined in `dashboard/core/rules/po_rules.py` (scope 15) — `PO_TERMINATED_WITH_INVOICING`, `PO_LINE_NEVER_MATCHED`, `PO_LINE_MATCH_EXCEEDS_RECEIPT`, `PO_LINE_ARR_CALC_MISMATCH`, `PO_HDR_LINE_DATE_MISMATCH`, `PO_HDR_LINE_STATUS_MISMATCH`, and `PO_LINE_NO_ACCOUNT` were all removed (August 2026) per direct request, and the 3-tier unmatched-receipt-aging check was consolidated back into a single `PO_LINE_UNINVOICED_RECEIPT_OVER3M` (also August 2026), together dropping the count from 28. `PO_LINE_NEG_AMOUNT` and `PO_LINE_VOW_CALC_MISMATCH` were re-scoped from all rows to open POs (`status IN (O,N,A)`) only. 18 currently appear in `dq_results` against dummy data; `PO_LINE_UNINVOICED_RECEIPT_OVER3M` has an empty shared population on dummy data and so doesn't appear at all (see the "shared base population is empty" note above), which is expected engine behaviour, not a bug. Includes 4 cross-domain checks against Suppliers (`asuheader`)/GL (`aglaccounts`) — see PO Domain section above for the full list
