...
**Table-wide population — a second exception, this time for checks that aren't about house at all (August 2026).** House for `unit4_commitments`/`unit4_spend` is derived entirely from an unrelated field (Supplier ID — see "Not split by house" above), so partitioning checks like date validity, duplicate Contract Id, or the Commitments-vs-Spend reconciliation by house fragments one real number into three misleading partial slices — and can hide a genuine duplicate/mismatch between two rows that happen to resolve to different houses, since they'd never appear in the same per-house slice together. This was surfaced by the user directly (a `UNIT4_COMMIT_NOT_IN_CONTRACTS` result reading "1 flagged of 5 total" turned out to be one arbitrary house-slice of a much larger real number). `_ATAMIS_TABLEWIDE_CHECKS` in `data_engine.py` lists the affected check IDs (`UNIT4_COMMIT_DATE_INVALID`, `UNIT4_COMMIT_REMAINING_MISMATCH`, `UNIT4_COMMIT_DUP_ID`, `UNIT4_COMMIT_OVERSPEND`, `UNIT4_COMMIT_NOT_IN_CONTRACTS`, `UNIT4_COMMIT_VS_SPEND_MISMATCH`, `UNIT4_SPEND_NEGATIVE_POSTED`) — `UNIT4_COMMIT_NO_SUPPLIER_ID` and `UNIT4_COMMIT_SUPPLIER_ORPHAN` are deliberately excluded since they ARE about the field that determines house. `_atamis_tablewide_population()` mirrors `_atamis_existence_population()`'s shape: every house iteration except a new sentinel `'All'` returns empty, and `'All'` gets the full table (optionally blank-excluded via `_ATAMIS_TABLEWIDE_BLANK_COL` — `UNIT4_COMMIT_NOT_IN_CONTRACTS` excludes blank `u4_contract_id`, since a blank id has nothing to compare against and was being wrongly counted as an orphan rather than excluded as incomplete, a real bug found alongside the fragmentation issue). `'All'` was added to `HOUSE_HEX` (violet) and `ATAMIS_HOUSES`, and the Atamis tab renders it as a third section, "Contract Commitments & Spend — Table-Wide Checks", alongside the resolved (HOC/HOL) and Unresolved Records sections.
...
- **DQ scorecard split into three sections** (August 2026, per direct request): resolved (HOC/HOL, genuinely comparable), Unresolved Records (`Unknown` — couldn't be matched to Unit4 at all, not a third house), and a new Table-Wide Checks section for the 7 Unit4 Commitments/Spend checks whose subject has nothing to do with house (see "Table-wide population" above) — surfaced when a `UNIT4_COMMIT_NOT_IN_CONTRACTS` result ("1 flagged of 5 total") turned out to be a fragment of a much larger real number, plus a genuine bug where a blank Contract Id was being counted as an orphan rather than excluded as incomplete.
- Tab (`dashboard/tabs/atamis.py`) built with a distinct dark-navy/Parliament-green visual identity: hero, HOC/HOL/Joint organisation split, a flagship Cross-System Reconciliation section (supplier overlap bar + 4 cross-system stat cards), top-15 contracts by value, contract lifecycle, contract financials, then the three-section DQ scorecard/grid described above
