...
- 24 checks in `dashboard/core/rules/po_rules.py` (scope 15), including 4 cross-domain checks against Suppliers (`asuheader`)/GL (`aglaccounts`), 6 added after direct pushback on real-data behaviour (`PO_DUP_HEADER`, `PO_FUTURE_ORDER_DATE`, `PO_HDR_LINE_DATE_MISMATCH`, `PO_TERMINATED_WITH_INVOICING`, `PO_ARR_EXCEEDS_AMOUNT`, `PO_LINE_NO_CATEGORY`), and 5 more added from the user's own manual Excel exploration of real PO lines (`PO_LINE_NEVER_MATCHED`, `PO_LINE_MATCH_EXCEEDS_RECEIPT`, `PO_LINE_INVOICED_AHEAD_OF_RECEIPT`, `PO_LINE_AMENDED_VALUE_MISMATCH`, `PO_LINE_STALE_UNRESOLVED`) — see PO Domain section above for the full list
