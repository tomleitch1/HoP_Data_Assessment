...
- `atamis_contracts` carries its own `Organisation` field (`HOC`/`HOL`/`Joint`). `HOC`/`HOL` map directly. `Joint` contracts are **resolved further** (August 2026, per direct request): the contract's `Supplier` name is matched against `asuheader.apar_name` (both houses) to determine which specific house that supplier belongs to, and the contract's `house` is reassigned to that house. This is a **name match, not an ID join** — `atamis_contracts` has no supplier ID field, only a free-text name — so real supplier names that differ slightly in formatting between Atamis and Unit4 (e.g. `"LTD"` vs `"LIMITED"`) will not resolve. A `Joint` contract whose supplier can't be matched in Unit4 at all is tagged `'Unknown'`, same meaning as every other unresolvable reference in this domain, rather than staying mislabeled as `'Joint'`. **Net effect: `'Joint'` never appears as a resolved `house` value** — every Atamis-DQ-scorecard house bucket is now `HOC`, `HOL`, or `Unknown` only. The raw `Organisation` field itself is untouched by this and still drives the tab's own "Contracts by Organisation" chart (`dashboard/tabs/atamis.py` groups on the raw field, not the resolved `house`, precisely so that genuine three-way split isn't lost) — that's a deliberately separate concept from the resolved house used for DQ checks.
