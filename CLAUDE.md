...
- `asutrans` structure: the SQL extract already filters `status != 'C'`. Each row is a real transaction record. The same invoice allocation can appear as multiple rows with **different status values** (N=open, P=payment proposal, R=on payment run, I=instruction sent) representing payment lifecycle stages — these are all distinct rows and should all be shown. Do NOT try to deduplicate them. The unique row identifier confirmed by Parliament data is `(client, apar_id, voucher_no, sequence_no)` but even this may not be unique if statuses vary. **For balance calculations**: sum `rest_amount` directly across all rows.
- **AP aging analysis** (`build_aging_analysis`) filters to `status.isin(['N','R','I'])` and works correctly — the aging table shows each row once with no phantom duplicates. DQ checks that use `status != 'C'` may see multiple status rows for the same invoice allocation.
- **Known issue — `get_failing_records` phantom duplicates**: for asutrans-based DQ checks, `get_failing_records` was observed creating multiple identical copies of source rows that do not exist in the source CSV (e.g. 1 source row → 5 modal rows, all identical). Root cause was not identified despite extensive investigation. The `AP_OVERDUE` check was removed because of this — timeliness is covered by the aging analysis instead. If adding new asutrans DQ checks, validate the modal table row count against the source CSV immediately.
