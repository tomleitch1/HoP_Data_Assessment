...
- 19 checks in `dashboard/core/rules/po_rules.py` (scope 15), including 4 cross-domain checks against Suppliers (`asuheader`)/GL (`aglaccounts`) and 6 more added after direct pushback on real-data behaviour (`PO_DUP_HEADER`, `PO_FUTURE_ORDER_DATE`, `PO_HDR_LINE_DATE_MISMATCH`, `PO_TERMINATED_WITH_INVOICING`, `PO_ARR_EXCEEDS_AMOUNT`, `PO_LINE_NO_CATEGORY`) — see PO Domain section above for the full list
