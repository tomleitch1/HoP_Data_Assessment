...
**The Export button is fixed modal chrome, not per-content — `modal-export-context` (a `dcc.Store`) tracks what to export (fixed August 2026).** `modal-overlay`'s header (with its `btn-export-modal` button) is defined once in the global layout and reused by every modal-populating callback — the DQ drill-down (`handle_modal_logic`), the Atamis Organisation Field Reliability cell click (`handle_atamis_org_reliability_click`), and the PO tab's "View all suppliers & contracts" button (`handle_po_leakage_view_all`). `export_modal_to_csv` originally derived what to export purely from DQ-modal state (`dim-widget-chart` clickData / `dim-results-table` active_cell) — so clicking Export while viewing an Organisation Field Reliability or PO leakage modal silently did nothing, since neither of those triggers populates that state. Found directly by the user testing the Organisation Field Reliability export. Fixed by adding `modal-export-context` (`dcc.Store`) to the layout: every modal-populating callback (including `close_modal`, which clears it) now also writes `{'type': 'dq'|'atamis_org_rel'|'po_leakage', ...identifying fields}`, and `export_modal_to_csv` reads that Store via `State` and branches on `type` instead of re-deriving context from click state. Verified via three raw `_dash-update-component` round trips (open each of the three modal kinds, capture the returned `modal-export-context.data`, replay it as the export click's `State`) — each produced real CSV content, including a regression check that the original DQ-modal export path still works unchanged.
...
Each dimension widget header also shows per-house score chips (`_house_score()` in `dimensions.py`). Each chip is a neutral grey badge (`#F1F5F9` background) with a house-coloured border and the error rate rendered in the RAG colour for that score. A gradient vertical divider separates each chip from the next. Only "avg error" label appears once between/after the chips.

**`render_dimension_widget()` is generic over however many house values are actually present** (August 2026 fix) — it was originally hardcoded to exactly `HOC`/`HOL`, which crashed (`ValueError: max() iterable argument is empty`) the first time it was called against a `dq_results` slice where neither literal string was present (Atamis's "Unresolved Records" section, house `'Unknown'` only — see Atamis Domain section below). Both `render_dimension_widget()`'s bar chart and its header score chips now iterate `houses = sorted(sub['house'].dropna().unique())` dynamically rather than a hardcoded `['HOC', 'HOL']`/`_house_score(hoc_error, 'HOC')` pair. Every other domain still only ever has HOC/HOL present, so this is a no-op behavioural change for them.
...
**Bug found on real data: Contract Award Value showed £0 for every row (fixed same day).** The user reported this immediately after testing against real Parliament data, despite the underlying contracts genuinely having an award value. Root cause: `contract_id` is not in the engine's global string-cleanup list (only `contract_ref`/`u4_contract_id` are — see `load_data()`'s `string_cols`), so `apodetail.contract_id` keeps any raw formatting (e.g. trailing whitespace) exactly as extracted, while `atamis_contracts.contract_ref` *is* cleaned. Joining an uncleaned `"C100 "` against a cleaned `"C100"` silently matches nothing, and `.sum()` over an all-NaN group returns `0`, not `NaN` — so a total join failure looked identical to "no award value recorded" rather than an obvious error. Fixed by stripping `contract_id` on both sides explicitly before the join (mirroring the exact normalisation `_atamis_contract_ref_not_in_po()` in `atamis_rules.py` already uses for the same PO↔Atamis join elsewhere). Verified via a forced synthetic case with a trailing space on the PO side only — Award Value now resolves correctly.

**Multi-contract suppliers — resolved by splitting into two frames (`contract_leakage_summary` / `contract_leakage_detail`), per direct request after the user asked how this should be presented.** The first version concatenated a supplier's contract refs into one string and summed their award values together — ambiguous and potentially misleading when a supplier has more than one, unrelated contract. Now:
- `contract_leakage_summary` — one row per supplier (tagged/untagged £, untagged %) — drives the card's headline stats and top-10 preview.
- `contract_leakage_detail` — **one row per (supplier, contract) pair** — each contract's own Award Value and tagged spend shown against *that* contract specifically, with the supplier's total untagged spend repeated on every one of their rows (clearly labelled as a supplier-level figure, not attributed to any single contract) — drives the modal.

**Modal added, replacing the top-25 cap (per direct request — "needs to be a modal which shows everything").** The card itself only ever previews the top 10 suppliers by untagged £; a "View all suppliers & contracts" button (`btn-po-leakage-view-all`) opens the same `modal-overlay`/`modal-title`/`modal-content` shell used by every other drill-down in the app, via `allow_duplicate=True` (`handle_po_leakage_view_all` in `app.py`, same pattern as `handle_atamis_org_reliability_click`). The modal shows the full `contract_leakage_detail` frame — every row, not truncated — in a `dash_table.DataTable` with native sort/filter/pagination, so "everything" is actually reachable rather than silently cut off. `app.py` recomputes `_po_compute_metrics(frames)` fresh on each click rather than threading the metrics dict through Dash state.

**Known dummy-data limitation:** `generate_po_dummy_data.py` and `generate_atamis_dummy_data.py` were built independently with disjoint `contract_id`/`contract_ref` pools (same root cause as the PO↔Atamis cross-domain ID mismatch already documented elsewhere) — so on dummy data every row's Contract Award Value still shows as £0/None, even after the whitespace fix, since the underlying values simply don't overlap between the two generators. The tagged/untagged split itself is computed correctly regardless (verified via forced synthetic cases with matching and non-matching contract refs). Real Parliament data pulls PO and Atamis from the same live systems, so this should not occur there — confirmed fixed once tested against real data.
...
Atamis is Parliament's procurement/contracts system, extracted as four files (added August 2026), plus two further files added later the same month once the user supplied them. Two of the original four are Atamis's own data (contracts, suppliers); the other two are Unit4/Agresso views of the same contract spend, pulled in for reconciliation against the Atamis side and against each other.

| File on Parliament laptop | Source system | Frame key | Scope |
|---|---|---|---|
| `contracts_report.csv` | Atamis | `atamis_contracts` | 30 |
| `contract_total_commitments.csv` | Unit4 (Agresso view #1) | `unit4_commitments` | 31 |
| `contracts_spend_details.csv` | Unit4 (Agresso view #2) | `unit4_spend` | 32 |
| `supplier_data_report.csv` | Atamis | `atamis_suppliers` | 33 |
| `unit4_open_contracts.csv` | Unit4 (HAIS master of open contracts) | `unit4_open_contracts` | — reference table only, no scope/checks of its own |
| `hol_unit4_spend.csv` | Unit4 (HAIS, HOL only) | `hol_unit4_spend` | — reference table only, no scope/checks of its own |

All six live in `data/atamis/` on both laptops (in `SUBDIR['atamis']` in `data_engine.py`, keyed by their real filenames, not by table name). The last two are pure reference/lookup tables consumed by other checks' population logic — no DQ checks are defined directly on them (not requested), and they get no `house` column of their own from `_derive_atamis_houses()`.

### unit4_open_contracts.csv — restricting Commitments/Spend checks to currently-open contracts
This is a master list of currently open Unit4 contracts (from HAIS) — a rich file (contract manager, dates, current/award value, supplier, etc.) but critically **it has no separate status column**: a contract's own existence as a row in this file is what "open" means. A contract not in this file is closed/historical.

Per direct request, once this file became available, the Unit4 Commitments/Spend reconciliation checks were changed to stop treating a *closed* contract's records as a data quality issue — a closed contract naturally has no reason to still reconcile cleanly against Atamis, so flagging it was noise. `_ATAMIS_OPEN_ONLY_CHECKS` in `data_engine.py` lists the affected checks: every `UNIT4_COMMIT_*`/`UNIT4_SPEND_*` check, plus `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS` (the Atamis-side check asking the same "does this trace to a real Unit4 record" question in the other direction). `_atamis_filter_open_only(h_df, table, frames)` is applied as a post-filter after each check's normal population is computed — for `atamis_contracts` it filters on `contract_ref`, for every other table on `u4_contract_id` — against the set built by `_atamis_open_contract_refs(frames)`. **If `unit4_open_contracts.csv` isn't loaded at all, the filter no-ops** (returns the population unfiltered) rather than excluding everything — deliberately safe against the file being absent (e.g. older data, or a tab-scoped load that hasn't picked it up).

Both `_atamis_filter_open_only` and `_atamis_open_contract_refs` are hashed into `_engine_sig()` alongside `_derive_atamis_houses`/`_build_unit4_contract_refs` — same reasoning as before: they're called *from* `run_dq_analysis` but are separate function bodies, so an edit to just their logic wouldn't otherwise bust the per-check cache.

### hol_unit4_spend.csv — a materiality gate on HOL's GL Contract Number signal
HOL has no Commitments extract of its own (see `unit4_contract_refs` below) — its only prior signal was a bare Contract Number code from the GL dimension value list, with no financial data attached. `hol_unit4_spend.csv` (HOL's own contract-spend report, sent to Atamis from HAIS) is just two columns — `Contract Number` / `Amount`, renamed to `u4_contract_id` / `amount` for consistency with every other Unit4-side table — but it's HOL's only source of real financial activity against a contract reference.

Per direct request: a HOL GL Contract Number dimension code should only count as a genuine Unit4-side signal if there's actual recorded spend against it — a code with no spend (or that's simply absent from `hol_unit4_spend.csv` entirely) is presumed dormant/unused rather than a real link. This is enforced once, at the source: `_build_unit4_contract_refs()` in `data_engine.py` sums `hol_unit4_spend` by `u4_contract_id` and drops any HOL GL-dimension row whose total spend isn't `> 0`, **before** that row ever enters `unit4_contract_refs` — so every consumer of that shared frame (`UNIT4_COMMIT_NOT_IN_CONTRACTS`, `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS`, and the Organisation Field Reliability card, which reads the same frame) automatically inherits the gated signal without needing to duplicate the logic. If `hol_unit4_spend` isn't loaded at all, the gate is skipped entirely (every HOL GL reference counts, matching the pre-existing behaviour) rather than silently excluding everything.

Verified via a forced synthetic case: three HOL GL dimension codes (one with real spend, one entirely absent from the spend file, one present but with `Amount = 0`) — confirmed only the one with genuine positive spend survives into `unit4_contract_refs`. Also verified the open-only filter end-to-end through `run_dq_analysis`: a synthetic overspent-but-closed commitment correctly drops out of `UNIT4_COMMIT_OVERSPEND`'s population entirely (rather than being counted as a false failure), and an Atamis contract with no commitment match is only flagged by `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS` if it's itself open.
...
- `atamis_contracts` carries its own `Organisation` field (`HOC`/`HOL`/`Joint`). `HOC`/`HOL` map directly. **Every other contract** (labelled `Joint`, blank, or any unexpected value) is **resolved further** (August 2026, per direct request) via the confirmed Contract Reference == Contract Id join, **not** a name match — a name-match version (matching the contract's `Supplier` field against `asuheader.apar_name`) was built first and explicitly rejected as not robust, since real supplier names commonly differ in formatting between Atamis and Unit4 (e.g. `"LTD"` vs `"LIMITED"`) and wouldn't reliably resolve. The confirmed approach instead: look up the contract's `Contract Reference` in `unit4_commitments.u4_contract_id` (computed first in `_derive_atamis_houses`, before `atamis_contracts`, since this depends on it) and adopt *that* commitment's already-resolved house. Reasoning, per the user directly: a commitment always pays one specific supplier, and that supplier belongs to exactly one house, so even though Atamis's own Organisation field says `Joint` (or is simply wrong/blank), the underlying spend has a real house once traced through to who's actually being paid. **This was originally scoped to `house == 'Joint'` only** — found directly by the user on real data (a contract with a confirmed-resolvable Commitments/Supplier chain was still showing `Unknown`) that a contract with a blank/invalid Organisation value skipped the lookup entirely, even though the identical resolution would have worked. Widened to `~house.isin(['HOC', 'HOL'])` — i.e. every contract not already cleanly resolved — so the same lookup now applies regardless of *why* the Organisation-based resolution didn't land cleanly. A contract is tagged `'Unknown'` only if no matching commitment record exists at all, or that commitment's own supplier doesn't resolve either — same meaning as every other unresolvable reference in this domain, not a gap to guess at. **Net effect: `'Joint'` never appears as a resolved `house` value** — every Atamis-DQ-scorecard house bucket is `HOC`, `HOL`, or `Unknown` only. The raw `Organisation` field itself is untouched by this and still drives the tab's own "Contracts by Organisation" chart (`dashboard/tabs/atamis.py` groups on the raw field, not the resolved `house`, precisely so that genuine three-way split isn't lost) — that's a deliberately separate concept from the resolved house used for DQ checks. Against dummy data, most unresolved contracts (8 of 11 `Joint` ones) have no matching commitment record at all and so resolve to `Unknown` — a dummy-data coverage gap (only ~35% of contracts get a commitment link regardless of house), not a logic bug; real-data behaviour depends on how many contracts have Unit4 commitment activity. `scripts/diagnose_atamis_house.py` prints the full resolution chain (organisation, matched commitment, that commitment's own supplier/house) for every contract still `Unknown`, to distinguish "no matching commitment at all" from "matched, but that commitment's own supplier doesn't resolve either" from a formatting mismatch.
- `atamis_suppliers`, `unit4_commitments`, and `unit4_spend` have no house field at all. House is derived by matching their supplier identifier against `asuheader.apar_id` (checking HOC first, then HOL): `Creditor Ref` for suppliers, `Supplier ID` for commitments. `unit4_spend` has no supplier identifier of its own, so it inherits its house transitively from its matched `unit4_commitments` row via `u4_contract_id`.
- A row whose identifier matches neither house is tagged `'Unknown'` — that mismatch is itself the condition several DQ checks test for (`ATAMIS_SUPPLIER_NOT_IN_UNIT4`, `UNIT4_COMMIT_SUPPLIER_ORPHAN`, `UNIT4_SPEND_CONTRACT_ORPHAN`), not a gap to paper over with a guessed default.
- House derivation always recomputes fresh on every `load_data()` call (not persisted through the per-table pickle cache), since it depends on `asuheader` and `unit4_commitments`, which may change independently of the Atamis files' own cache freshness. Cheap — a few thousand rows.
- **Blank-key merge in the Joint-resolution lookup itself (fixed, same session as the widening above).** The lookup in `_derive_atamis_houses()` merges unresolved contracts against `unit4_commitments` on `contract_ref`/`u4_contract_id` — same blank-matches-blank pandas merge hazard already found and fixed in `ATAMIS_CONTRACT_VALUE_MISMATCH` (see "Blank-key merges" below), here in the code that decides house itself rather than in one check's logic. Without the fix, a contract with a blank `contract_ref` could spuriously match a commitment that also has a blank/missing `u4_contract_id` and adopt *that* commitment's house, instead of correctly staying `Unknown` for lack of anything to look up. Fixed by baselining every unresolved row to `'Unknown'` first, then only attempting the lookup for rows with a non-blank `contract_ref`, against a `commit_house` table that itself excludes blank `u4_contract_id` rows. (An earlier draft of this fix simply excluded blank-ref rows from the lookup without the baseline step — that left them at whatever `house` already held, which is `'Joint'` for an Organisation of `'Joint'`, not `'Unknown'` — caught by a synthetic test before shipping.) Verified via synthetic cases: blank-ref + blank-commitment-id no longer spuriously resolves; a genuine contract_ref-to-commitment match still resolves correctly to that commitment's own house.
...
**Existence-check population — a deliberate exception to per-house partitioning.** `ATAMIS_SUPPLIER_NOT_IN_UNIT4`, `UNIT4_COMMIT_SUPPLIER_ORPHAN`, and `UNIT4_SPEND_CONTRACT_ORPHAN` all test "this row's own derived house is `Unknown`". Naively partitioning by house would produce a trivial 0%-failing row under `HOC`/`HOL`/`Joint` (an unresolved row can never appear in a resolved house's population) alongside the one real 100%-failing row under `Unknown` — noisy and not a genuine rate. `_atamis_existence_population()` in `data_engine.py` fixes this: for these three check IDs, every house iteration *except* `'Unknown'` returns an empty population (skipped entirely), and the `'Unknown'` iteration's population is the **full non-blank identifier population** (not house-filtered), so the check's failing/total ratio is a real percentage of the whole population, reported as a single `dq_results` row tagged `house='Unknown'`. `ATAMIS_CONTRACT_REF_NOT_IN_PO` does not use this pattern — `HOC` and `Joint` are both genuine, meaningful populations for that check (PO is HoC-only, so `HOL` naturally yields zero and is skipped by the ordinary per-house filter).

**Table-wide population — introduced then reverted (August 2026).** House for `unit4_commitments`/`unit4_spend` is derived entirely from an unrelated field (Supplier ID — see "Not split by house" above), so a `'All'` pseudo-house and a separate "Table-Wide Checks" section were briefly added for 7 checks whose subject has nothing to do with house (date validity, duplicate Contract Id, the Commitments-vs-Spend reconciliation, etc.) — see git history (`_ATAMIS_TABLEWIDE_CHECKS`, `_atamis_tablewide_population()`) if this reasoning is ever revisited. Once the Joint-resolution fix above was widened to cover every unresolved contract (not just ones literally labelled `Joint`), essentially every real row resolves cleanly to `HOC` or `HOL`, so the separate section was no longer earning its complexity — removed per direct request, and all 7 checks now report through the normal per-house scorecard/grid like every other check in this domain. The one real fix that came out of the table-wide detour is kept: `UNIT4_COMMIT_NOT_IN_CONTRACTS` (and, by the same reasoning, `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS`) still excludes rows with a blank `u4_contract_id`/`contract_ref` from the population — a blank id has nothing to compare against and was being wrongly counted as an orphan rather than excluded as incomplete — now applied per-house via an ordinary `elif check_id in (...)` branch in `run_dq_analysis()`/`get_failing_records()` instead of the removed table-wide helper. `ATAMIS_HOUSES` is back to `['HOC', 'HOL', 'Joint', 'Unknown']` and `HOUSE_HEX['All']` was removed.
...
### The join keys
- **`contracts_report.Contract Reference`** (Atamis, e.g. `FWK1128-MEPFS1037`) joins directly to **`po_detail_HOC.contract_id`** — confirmed by Parliament. HOL contracts have no PO to match against (PO is HoC-only), so `ATAMIS_CONTRACT_REF_NOT_IN_PO` is scoped to resolved `house == 'HOC'` only (`'Joint'` never appears as a resolved house — see "Not split by house" above).
- **`contracts_report.Contract Reference`** also joins directly to **`contract_total_commitments.Contract Id`** — confirmed directly by the user. (Earlier in this build these were assumed to be two unrelated identifier schemes, since the two sample values looked structurally different — `FWK1128-MEPFS1037` vs `ARC1002` — but they are in fact the same join key across both systems. `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS`, `UNIT4_COMMIT_NOT_IN_CONTRACTS`, and `ATAMIS_CONTRACT_VALUE_MISMATCH` implement this join both directions, plus a value-agreement check between Atamis's `Total Award Value` and the Commitments view's `Contract Award Amount`.)
...
- **HOL has no equivalent Commitments extract worth using** (confirmed by the user directly, August 2026) — `contract_total_commitments.csv` resolves almost exclusively to HOC in real data (see the earlier finding above: 4,143 of 4,148). HOL masters its contract reference completely differently: it lives in **`gl_dimension_values_HOL.csv`** (`agldimvalue`), specifically the row where **`dim_position == '5'`** and **`dim_description == 'Contract Number'`** — `dim_value` on that row *is* the contract reference, joining directly to `atamis_contracts.contract_ref` the same way `unit4_commitments.u4_contract_id` does for HOC. This applies to **both directions** of the Contract ↔ Commitment reconciliation (`ATAMIS_CONTRACT_NOT_IN_COMMITMENTS` and `UNIT4_COMMIT_NOT_IN_CONTRACTS`), via the shared `unit4_contract_refs` frame — see `_build_unit4_contract_refs()` in `data_engine.py` below.

### unit4_contract_refs — a shared frame for both directions of the Contract ↔ Commitment join
Two checks need "what Unit4-side contract references genuinely exist, across both houses' completely different sources" — `UNIT4_COMMIT_NOT_IN_CONTRACTS` (does this Unit4 reference trace to a real Atamis contract?) and `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS` (does this Atamis contract trace to a real Unit4 reference?). Rather than teaching every other Commitments check (`REMAINING_MISMATCH`, `DATE_INVALID`, `DUP_ID`, `OVERSPEND`, `SUPPLIER_ORPHAN`) about a second source it doesn't have data for, `_build_unit4_contract_refs(frames)` in `data_engine.py` builds one dedicated frame, `unit4_contract_refs`, that both checks share:
- A full copy of `unit4_commitments` (same columns — so `UNIT4_COMMIT_NOT_IN_CONTRACTS`'s existing lambda and modal enrichment work unchanged for HOC, with real `contract_title`/`supplier_name`/etc.), tagged `_source = 'unit4_commitments'`.
- Plus synthetic HOL rows appended from `agldimvalue` (`dim_position == '5'`, `dim_description == 'Contract Number'`, `dim_value` → `u4_contract_id`), with every other commitment-specific column left blank (HOL has no equivalent rich data) and tagged `_source = 'gl_dimension_values'` — **gated by real spend** (see "hol_unit4_spend.csv — a materiality gate" above): a HOL GL dimension code with no recorded spend in `hol_unit4_spend.csv` never makes it into this frame at all.

`UNIT4_COMMIT_NOT_IN_CONTRACTS`'s own `table` field points at `unit4_contract_refs` (its population is this frame, house-filtered as normal). `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS` keeps `atamis_contracts` as its own table/population (unchanged — every Atamis contract, both houses) but its `joined_table` and lambda (`_atamis_contract_not_in_commitments`) now check membership against `unit4_contract_refs['u4_contract_id']` instead of `unit4_commitments['u4_contract_id']` directly — so a HOL contract is correctly tested against HOL's real source (the GL dimension value) rather than against a Commitments extract that was never going to contain it. Every other check on `unit4_commitments` is untouched by any of this.

`_build_unit4_contract_refs` is called from `load_data()` right after `_derive_atamis_houses()` and, like it, is hashed into `_engine_sig()` so a future edit to this logic correctly busts the per-check cache for both checks (same reasoning as the house-derivation cache gap found earlier); the per-check cache's `rel_fps` tracking also covers `unit4_contract_refs` appearing as either a check's own `table` or its `joined_table`. `python run_dashboard.py atamis` (tab-scoped mode) now also loads `gl_dimension_values` for this reason. The modal evidence for `UNIT4_COMMIT_NOT_IN_CONTRACTS` shows `house` and `UNIT4_COMMITMENTS.source` so it's visible at a glance which of the two source systems a flagged row came from, rather than implying HOL has the same rich commitment data HOC does.

**Known dummy-data limitation:** dummy data's HOL `dim_position == '5'` attribute (`LCTPT`) is labelled `'Counterpart'`, not `'Contract Number'` — a pre-existing invented label from before this real-data fact was known. `_build_unit4_contract_refs`'s HOL population is therefore empty on dummy data (0 rows, filtered out entirely), so neither check picks up any HOL-sourced GL dimension matches locally — confirmed as a generator gap, not a logic bug, via forced synthetic cases for both checks (HOC- and HOL-sourced rows resolving and flagging correctly in both directions). Not fixed in the generator, since doing so would mean either repurposing the existing `LCTPT`/`Counterpart` attribute (removing dummy Counterpart coverage) or fabricating a second unrelated attribute at the same `dim_position` that no real client would actually have.

### DQ checks (24 total, `dashboard/core/rules/atamis_rules.py`)
**Naming convention: the check ID prefix reflects which system the row itself belongs to, not which table it's compared against.** Checks on `contracts_report`/`supplier_data_report` (genuinely Atamis's own data) are prefixed `ATAMIS_CONTRACT_*`/`ATAMIS_SUPPLIER_*`. Checks on `contract_total_commitments`/`contracts_spend_details` are prefixed `UNIT4_COMMIT_*`/`UNIT4_SPEND_*` — **not** `ATAMIS_COMMIT_*`/`ATAMIS_SPEND_*` as originally built. Renamed August 2026 per direct request after the original naming was found misleading in the modal (it implied this data came from Atamis, when both of these files are Unit4/Agresso views). The internal frame keys were renamed to match (`atamis_commitments` → `unit4_commitments`, `atamis_spend` → `unit4_spend`), so the modal's column-prefix labels for the generic (non-early-return) checks now correctly read `unit4_commitments.*`/`unit4_spend.*` instead of `atamis_commitments.*`/`atamis_spend.*`. `contracts_report`/`supplier_data_report`'s own frame keys (`atamis_contracts`/`atamis_suppliers`) are correctly named and unaffected.
...
- `ATAMIS_SUPPLIER_NOT_IN_UNIT4` — Atamis supplier with no matching Unit4 supplier record. (The reverse-direction check, `UNIT4_SUPPLIER_NOT_IN_ATAMIS`, was removed per direct request — see "Removed checks" below.)
- `ATAMIS_CONTRACT_REF_NOT_IN_PO` — Atamis contract with no matching PO line.
- `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS` / `UNIT4_COMMIT_NOT_IN_CONTRACTS` — contract existence in each direction, via the confirmed Contract Reference / Contract Id join (see The join keys, above). The reverse direction (a Unit4 commitment with no Atamis contract at all) is more surprising, so it's High rather than Medium severity.
- `ATAMIS_CONTRACT_VALUE_MISMATCH` — for a matched contract, Atamis's own `Total Award Value` disagrees with the Commitments view's `Contract Award Amount` by more than 2% or £1 (whichever is larger). **VAT-variation exemption (added August 2026, per direct request):** a ~20% difference where one side is exactly the other × 1.2 (e.g. Unit4 `112,500` vs Atamis `135,000`) is one side recorded VAT-inclusive and the other VAT-exclusive — not a genuine data issue — and is excluded from the flag even though it exceeds the normal tolerance. Checked in both directions (either system could be the VAT-inclusive one), with a small rounding allowance (0.5% of the Atamis value or £1, whichever is larger).
- `ATAMIS_CONTRACT_DATE_MISMATCH` (added August 2026, direct request) — for a matched contract, Atamis's own `Start Date`/`End Date` disagree with the Commitments view's `Contract Date From`/`Contract Date To`. Same shape as `ATAMIS_CONTRACT_VALUE_MISMATCH` — same blank-`contract_ref` exclusion to avoid a spurious blank-matches-blank merge, unmatched contracts left unflagged (nothing to compare), each side's own blank date excluded from its half of the comparison so a genuinely missing date isn't misread as a mismatch (that's `ATAMIS_CONTRACT_NO_DATES`'s own concern). **HOC only** — HOL's contract reference comes from the GL dimension value (`unit4_contract_refs`, see above), which has no date fields of its own; its `period_from`/`period_to` describe when that dimension code was valid for GL postings, not the contract's actual start/end date, so it isn't a safe substitute and wasn't used. Modal evidence enriches with the matched commitment's own dates via an explicit named join (same pattern as `ATAMIS_CONTRACT_VALUE_MISMATCH`), so the disagreement is directly visible.
- `UNIT4_COMMIT_SUPPLIER_ORPHAN` / `UNIT4_SPEND_CONTRACT_ORPHAN` — Unit4-side records that don't resolve against the supplier master / the other Unit4 view.
- `UNIT4_COMMIT_VS_SPEND_MISMATCH` — the two Unit4 views' own `Posted` figures disagree by more than a materiality threshold on the same contract; Parliament flagged this as a known possible discrepancy area between the systems.
- `UNIT4_COMMIT_OVERSPEND` — a contract posted beyond its authorised `Contract Amount Limit`.
- `UNIT4_COMMIT_REMAINING_MISMATCH` — Remaining Amount is expected to equal Amount Limit minus Committed Amount. **Revised August 2026, per direct request:** "Committed Amount" here is the Spend Details view's own `Amount (C)` field (`contracts_spend_details.csv` / `unit4_spend.amount_c`), summed per contract, **not** the Commitments view's own `committed_amount` column — was originally `Amount Limit − Posted Amount`, both fields sourced from `unit4_commitments` itself. Now a genuine cross-table check (`_unit4_commit_remaining_mismatch(df, frames)` in `atamis_rules.py`, table `unit4_commitments` joined to `unit4_spend`); a commitment with no matching Spend Details record at all is left unflagged, same "nothing to compare against" convention as every other cross-system check in this domain. Applies uniformly across every house bucket the check appears under, including Unknown, since it's one shared function. **Known dummy-data limitation:** shows ~100% failing on dummy data since `generate_atamis_dummy_data.py` generates `unit4_commitments.remaining_amount`/`amount_limit` and `unit4_spend.amount_c` independently, so the exact reconciliation formula essentially never coincidentally holds — confirmed as a generator gap, not a logic bug, via a forced synthetic case (matching and mismatching contracts, plus one with no Spend Details match at all, all evaluated correctly).
- `UNIT4_COMMIT_VS_PO_MISMATCH` (added August 2026, direct request, after a back-and-forth to establish exactly what the user meant by "discrepancies in PO spend against contracts" — see below) — for a contract with PO lines tagged to it, the Commitments view's own `Posted Amount` disagrees materially with the sum of `apodetail.invoiced` across every PO line sharing that Contract Id. **Does not go via Atamis at all** — `unit4_commitments.u4_contract_id` and `apodetail.contract_id` are already the same identifier directly (the confirmed direct join `ATAMIS_CONTRACT_REF_NOT_IN_PO` also relies on), so Atamis's own contract master isn't needed for this specific comparison. Uses PO's `invoiced` field, not `amount` — "Posted" means an invoice has actually been processed through the ledger, which is what `invoiced` represents; `amount` is only the ordered/committed value (a different, not-yet-built comparison against Commitments' own `committed_amount`). `arr_amount` and `vow_amount` were both confirmed not relevant to this specific test. Unmatched contracts (no PO lines tagged at all) are left unflagged — nothing to compare against. Added to `_ATAMIS_OPEN_ONLY_CHECKS` alongside every other `UNIT4_COMMIT_*` check. Verified via a forced synthetic case (three contracts: matching, mismatching, and no PO match at all) — correctly flags only the mismatching one. Shows 0 failures on dummy data since PO's and Atamis's dummy generators use disjoint contract reference pools (same known cross-domain ID mismatch documented elsewhere) — no PO invoiced total ever resolves for a dummy Commitments contract, so nothing has anything to compare against.

**This check followed a separate design conversation, distinct from "Untagged Spend for Contracted Suppliers" (see PO Domain section above).** The user initially asked for PO-vs-contract discrepancy analysis, which was first built as the PO tab's own untagged-spend card — then the user paused and asked "what do I actually want to test," pointing out that card never actually joined to Unit4 Commitments or Atamis at all, just looked at PO internally. Talked through in plain conversation (not a build-first-ask-later approach) to reach three explicit decisions: (1) the real test is PO's actual spend vs Commitments' own `Posted Amount`, not just "does PO have a contract tag at all"; (2) keep both analyses — the untagged-spend card answers a tagging-completeness question, this new check answers a value-reconciliation question, genuinely different concerns; (3) `invoiced` (not `amount`) is the right PO field for "Posted", since Posted implies an actual ledger posting, and `vow_amount`/`arr_amount` were confirmed not relevant.

### Modal drill-down
Most checks fall through to `get_failing_records()`'s generic tail (no early-return block needed — same convention as every other domain's simple checks), which prefixes columns with the lowercase table name (e.g. `atamis_contracts.contract_ref`) and still narrows correctly via `get_check_columns()`'s substring-based `base_cols` matching in `app.py`. Seven checks have explicit early-return blocks (same reasoning as PO's cross-domain checks — the generic referential-integrity auto-join's only common column between these tables is `house`, which would dedupe the joined side down to one arbitrary row per house and attach it to every failing row, or — for `UNIT4_COMMIT_VS_PO_MISMATCH` — no common column with `apodetail` at all, which has no `house` column): `ATAMIS_CONTRACT_REF_NOT_IN_PO`, `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS`, `UNIT4_COMMIT_NOT_IN_CONTRACTS`, `ATAMIS_CONTRACT_VALUE_MISMATCH`, `ATAMIS_CONTRACT_DATE_MISMATCH`, `UNIT4_COMMIT_VS_SPEND_MISMATCH`, and `UNIT4_COMMIT_VS_PO_MISMATCH` (the value/date-comparison checks each enrich with the matched record's own figure from the other table, so the two systems' disagreement is directly visible rather than just implied).

**`UNIT4_SUPPLIER_NOT_IN_ATAMIS` was removed (August 2026) per direct request** — the check, its `_unit4_supplier_not_in_atamis` lambda, `get_check_columns()` entry, and `get_failing_records()` early-return block were all deleted from `atamis_rules.py`/`data_engine.py`. `ATAMIS_SUPPLIER_NOT_IN_UNIT4` (the other direction — an Atamis supplier with no matching Unit4 record) is unaffected and still live.

**Blank-key merges — a bug class found in `ATAMIS_CONTRACT_VALUE_MISMATCH` (August 2026), fixed there and pre-emptively in `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS`.** Any check that joins on `contract_ref`/`u4_contract_id` without first excluding blanks on both sides can produce a spurious match: a contract with no reference at all can pair up with an unrelated commitment record that also has a blank/missing Contract Id (blank equals blank in a pandas merge), producing a fake value comparison against a completely unrelated amount. Caught directly by the user, who noticed a flagged row with a blank Contract Reference that nonetheless showed a joined award amount — impossible unless the join itself was matching on the blank. Fixed by excluding blank keys from both the left (population-level, mirroring `ATAMIS_CONTRACT_REF_NOT_IN_PO`'s existing pattern) and right (inside `commit`/`commit_link`, before the merge) sides in both `_atamis_contract_value_mismatch()` (`atamis_rules.py`) and its `get_failing_records()` early-return block. `UNIT4_COMMIT_NOT_IN_CONTRACTS` was already safe from this specific failure mode (its own blank-`u4_contract_id` exclusion — see "Table-wide population" above for where that now lives), and `unit4_spend`'s own blank `u4_contract_id` rows are filtered at load time (the grand-total row), so `UNIT4_COMMIT_VS_SPEND_MISMATCH` was never exposed to it either.

**A stale-cache gap this bug surfaced (August 2026), now fixed.** The two house-derivation fixes above (Joint resolved via commitments, then corrected again to use Contract Reference/Contract Id rather than a name match) each changed which Atamis rows land in which house — but neither edit touched `run_dq_analysis`'s own source, and `_derive_atamis_houses()` is called from `load_data()`, not from `run_dq_analysis`. Since `_engine_sig()` (the per-check cache's freshness signal) only ever hashed `run_dq_analysis`'s source, changes to house derivation silently never busted the cache — the DQ scorecard kept serving pre-fix summary counts indefinitely (e.g. `ATAMIS_CONTRACT_VALUE_MISMATCH` showing "35 flagged" under `Unknown` long after the fix that shrank that population to 8 rows), while `get_failing_records()` — which is never cached — correctly showed the small, current result on every click. That mismatch (a scorecard number with no matching drill-down rows) is what surfaced it. `_engine_sig()` now also hashes `_derive_atamis_houses()`'s source, so any future edit to house derivation correctly invalidates every Atamis per-check cache entry. Verified: cleared the cache, re-ran, and confirmed every one of the 43 `dq_results` rows for this domain now has a `get_failing_records()` row count that matches its `failing` figure exactly.

### Organisation Field Reliability (volumetrics-only, added August 2026, per direct request)
A new card in `_compute_metrics()`/`_render_org_reliability()` (`dashboard/tabs/atamis.py`), placed directly under the existing "Contracts by Organisation" pie. Purely informational — no RAG scoring, no `dq_results` row — the user explicitly asked for this "as volumetrics", not a DQ check.

**The gap it closes:** `_derive_atamis_houses()`'s Joint-resolution lookup only cross-checks the Supplier ID / Contract Number chain for contracts whose Organisation *doesn't already* cleanly map to HOC/HOL (`needs_resolution = ~house.isin(['HOC','HOL'])`) — by design, a contract Organisation already calls `HOC` or `HOL` is trusted outright and never gets that lookup. That means the resolved `house` column used by every DQ check can never surface cases the user found directly on real data: a contract labelled `HOL` whose matched commitment's own Supplier ID is actually a HOC supplier, or a contract labelled `HOC` that also matches HOL's GL Contract Number dimension. This card runs the same underlying lookup **on every contract with a reference, regardless of what Organisation already says**, and cross-tabs the result against the raw Organisation field.

**Verdict logic (`org_reliability_detail`):** for each contract, look up every distinct `house` value its `contract_ref` resolves to across `unit4_contract_refs` (which already carries both signals — a commitment's own supplier-derived house, and a flat `'HOL'` tag for a GL Contract Number match):
- No `contract_ref` at all → `No Reference`
- Has a reference, but it matches nothing in either source → `No Match`
- Matches something, resolves to exactly one house → that house (`HOC`, `HOL`, or `Unknown` if the matched commitment's own supplier doesn't resolve either)
- Matches more than one distinct house across the two sources → `Conflicting` — the two sources disagree with *each other*, independent of what Organisation claims

**Row totals reconcile exactly with the Contracts by Organisation totals above (fixed August 2026, found directly by the user).** An earlier version silently excluded contracts with a blank `contract_ref` from the table and only reported their count as a single overall footnote number — which meant a given Organisation's row total in this matrix (e.g. HOC 485) didn't match its total in the pie above (HOC 486), by however many of that house's contracts had no reference. Fixed by giving `No Reference` its own verdict column instead of dropping those rows, so `org_reliability_detail()` now covers every contract, and every Organisation's row total matches the pie exactly. The one remaining, deliberate exception: the pie above is explicitly scoped to the three confirmed Organisation categories (`mix[mix['house'].isin(_ORG_ORDER)]` in `_render_org_split`) and omits the blank/unrecognised-Organisation `'Unknown'` row entirely, whereas this table keeps it as its own row — documented in the card's own footnote rather than silently causing a second mismatch.

**Reading the table:** rows = raw Organisation (HOC/HOL/Joint/Unknown), columns = derived verdict. Green = Organisation agrees with the resolved verdict. **Amber = Organisation says one house but the data resolves to the other** — a genuine label/data disagreement on a contract that wasn't even ambiguous by Organisation's own account. **Red = Conflicting** — the two underlying sources disagree with each other. Three neutral shades, darkest to lightest: dark grey = `Unknown` (matched something, but that record's own supplier is unresolved), mid grey = `No Match` (had a reference, matched nothing), light grey = `No Reference` (nothing to even check). All cells are solidly filled (added per direct request — green/Unknown/No Match originally showed as plain coloured text rather than a filled box like amber/red already had).

**Known dummy-data limitation:** since dummy HOL's `dim_position == '5'` attribute is still labelled `'Counterpart'` (not `'Contract Number'` — see `unit4_contract_refs` above), no dummy contract_ref ever carries a `'HOL'` signal from the GL dimension side, so the `HOL` and `Conflicting` columns always read 0 on dummy data. The `HOC`-labelled-but-resolves-to-`HOC`, and `HOL`-labelled-but-resolves-to-`HOC` cells (the pure commitments-chain signal) do populate correctly on dummy data and were used to verify the logic.

**Cells are clickable, opening the same modal used for DQ drill-downs (added August 2026, per direct request).** The user wanted to trace individual conflicting/mismatched records, not just see the summary counts. The per-contract logic behind the crosstab was pulled out of `_compute_metrics()` into two module-level functions in `dashboard/tabs/atamis.py` so both the summary and the drill-down use identical matching logic:
- `org_reliability_detail(frames)` — the full per-contract detail frame (one row per contract, including reference-less ones), with enrichment columns `commitment_id`/`commitment_supplier_id`/`commitment_supplier_name`/`commitment_house` (from the matched `unit4_commitments` row, if any) and `gl_dim_match` (bool — whether the reference also matches HOL's GL Contract Number dimension). `_compute_metrics()` now just crosstabs this frame's `_org_clean`/`_derived` columns for the summary card.
- `get_org_reliability_records(frames, organisation, verdict)` — filters that detail frame to one matrix cell; called by app.py's modal callback.

Each non-zero matrix cell is rendered as an `html.Button` (styled transparent/borderless so it still looks like a plain cell) with a pattern-matching id `{'type': 'atamis-org-rel-cell', 'org': ..., 'verdict': ...}` — zero-value cells stay as plain, unclickable text since there's nothing to drill into. `app.py`'s `handle_atamis_org_reliability_click` callback targets the *same* `modal-overlay`/`modal-title`/`modal-content` components as the DQ drill-down (`handle_modal_logic`), via `allow_duplicate=True` — same pattern already used by `close_modal` to reset those same outputs. This isn't a DQ check (no severity/RAG/thresholds), so the modal content is deliberately simpler than the DQ inspector: just a `dash_table.DataTable` of the underlying contracts, styled to match (dark `#1e1528` header, same cell/border colours), with columns tracing exactly which source(s) resolved each contract and to what house — so a `Conflicting` cell's rows show both a populated `commitment_house` *and* `gl_dim_match = Yes` side by side, making the disagreement directly visible rather than just implied by the cell's label. Verified via a raw `_dash-update-component` POST simulating a pattern-matching Input click (not just a rendered-HTML check, since this is the first pattern-matching-Input callback added to this codebase) — confirmed HTTP 200, correct title text, and correct underlying rows for both an agreeing cell (Organisation=HOC → resolves to HOC) and a disagreeing one (Organisation=HOL → resolves to HOC).

### Tab volumetrics must use the same source as the DQ checks (found and fixed, August 2026)
The "Contract Overlap: Atamis ↔ Commitments" reconciliation bar in `_compute_metrics()` (`dashboard/tabs/atamis.py`) computed its own matched/unmatched counts independently of the DQ checks — and had not been updated when `unit4_contract_refs` was introduced, so it kept checking every contract (both houses) against `unit4_commitments` alone. Since `unit4_commitments` is HOC-only in practice, every HOL contract with a populated reference showed as "Atamis only" regardless of whether it genuinely matched HOL's real source (the GL dimension value) — caught by the user directly comparing the widget's "Atamis only: 79" against the DQ checks' own failing counts across HOC/HOL/Unknown, which summed to only 63. Fixed by pointing both directions of the calculation (`commit_ids` for the Atamis-only/matched split, and the reverse `commit_not_in_contracts`/`commit_total_all`) at `frames['unit4_contract_refs']` instead of `frames['unit4_commitments']`, with the same blank-`u4_contract_id` exclusion the DQ checks use. Verified: `contract_commit_unmatched` now equals `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS`'s failing sum across every house exactly (48 = 48 on dummy data), and `commit_not_in_contracts` equals `UNIT4_COMMIT_NOT_IN_CONTRACTS`'s failing sum exactly (41 = 41). The card's subtitle was also updated to name both sources rather than only the Commitments view. This is a standing risk worth remembering: any DQ check with custom population logic that also has a volumetrics widget summarizing "the same thing" needs both updated together, or they silently drift apart — as happened here.

### Tab (`dashboard/tabs/atamis.py`)
Dark navy + Parliament green design (distinct from PO's teal), following the same hero/section-card/DQ-scorecard structure as every other tab: hero banner, an Organisation split (HOC/HOL/Joint — the only Atamis dataset with a genuine three-way house split, `contracts_report`'s own raw field, deliberately NOT the resolved `house` column — see "Not split by house" above), a **Cross-System Reconciliation** section (the flagship view — two side-by-side three-segment overlap bars, one for Atamis-only/matched/Unit4-only suppliers and one for the confirmed Contract Reference ↔ Contract Id linkage, plus five stat cards for the remaining cross-system checks including the award-value agreement check), top-15 contracts by award value, contract lifecycle (active/expired/expiring-within-90-days by `End Date`), a contract financials summary comparing the two Unit4 views, then the DQ section.

**DQ section is split into two, not one (August 2026, per direct request):** `render_tab()` partitions `dq_results` into `resolved_dq` (`house` in `HOC`/`HOL`) and `unresolved_dq` (`house == 'Unknown'`) before rendering. `resolved_dq` gets the standard `render_dimension_scorecard()`/`render_dimension_grid()` under a "Data Quality Checks" heading — genuinely comparable per-house DQ, exactly like every other domain. `unresolved_dq` gets its own **"Unresolved Records"** section (`_render_unresolved_section()`), reusing the same two shared components but under an explanatory banner making clear `'Unknown'` is not a third house — it's every record whose Creditor Ref/Supplier ID/Contract Reference didn't resolve to a real Unit4 record in either house, or (for contracts) a Joint contract whose supplier name didn't match anything. Rationale: showing `Unknown` as a peer column alongside HOC/HOL in the same scorecard implied it was a comparable segment of the migration population with its own DQ score, which misrepresented what it actually is. The checks themselves are unchanged — same population, same logic — only how the results are grouped and framed changed. (A third "Table-Wide Checks" section existed briefly for the 7 house-unrelated Unit4 Commitments/Spend checks — removed per direct request once the Joint-resolution widening made table-wide fragmentation unnecessary; see "Table-wide population" above.)

**Four checks excluded from the Unresolved Records section specifically (added August 2026, per direct request):** `_UNRESOLVED_EXCLUDED_CHECKS` in `dashboard/tabs/atamis.py` filters `ATAMIS_CONTRACT_NO_SUPPLIER`, `ATAMIS_CONTRACT_NO_REF`, `ATAMIS_CONTRACT_NO_DATES`, and `ATAMIS_CONTRACT_ORG_INVALID` out of `unresolved_dq` only — they're still scored normally under `resolved_dq` (HOC/HOL), where they're meaningful. Reasoning: a blank Contract Reference or an invalid Organisation value is very often the *exact reason* a contract couldn't be resolved to a house in the first place, so surfacing it again as its own failing check under Unknown mostly restates what the Organisation Field Reliability card's `No Reference` column and the house-resolution logic itself already say, rather than adding new information — noise on top of a section whose whole point is investigating the resolution gap, not re-explaining it.

**Real-data finding that prompted a closer look at this (August 2026):** running the diagnostic script below against real Parliament data showed HOC/HOL supplier IDs never collide (0 overlap, unlike the dummy data — see below), but two separate real findings emerged: (1) `contract_total_commitments.csv`'s Supplier IDs matched almost exclusively HOC suppliers (4,143 of 4,148, zero HOL) — worth confirming with Parliament whether this extract is scoped to HOC only or whether HOL genuinely has near-zero Unit4 commitment activity; (2) `supplier_data_report.csv`'s Creditor Ref only matched real Unit4 suppliers ~15% of the time (841 of 5,562) — inspecting the unmatched values showed many aren't IDs at all (`'ACCESS SYSTEMS (UK) LIMITED'`, `'see Addison Lee Limited'`, `'8723692 a'`) — a real Atamis data quality issue (wrong kind of value in the Creditor Ref field), not a matching bug. A follow-up check distinguishing "malformed/non-ID Creditor Ref" from "genuinely orphaned ID" has been discussed but not yet built.

### Diagnostic script (`scripts/diagnose_atamis_house.py`)
Read-only — makes no changes. Run directly (`python scripts/diagnose_atamis_house.py`) against whichever data is loaded (needs the full dashboard, not tab-scoped, so `asuheader` is available) to print: the raw overlap between HOC's and HOL's `apar_id` sets, and for `atamis_suppliers`/`unit4_commitments` a breakdown of each ID into only-HOC / only-HOL / in-both (genuine collision) / in-neither (orphan or malformed), plus sample values for both the unmatched IDs and real HOL `apar_id`s side by side so a formatting mismatch is visible at a glance. Was essential in distinguishing the dummy-data ID collision (fixable by regenerating dummy IDs, not yet done — see below) from the two genuinely different real-data findings above.
...
**Known dummy-data limitation, not yet fixed:** `generate_ap_dummy_data.py` numbers HOC's and HOL's dummy suppliers identically (`SUP0001`–`SUP0080` in both, independently) — confirmed via `diagnose_atamis_house.py` (80/80 collision, 0 HOL-only IDs). Since Atamis house resolution checks HOC's ID set first, this means no dummy Atamis/Commitments/Spend record can ever resolve to HOL — everything either matches HOC or falls to `Unknown`. Confirmed as a dummy-data-only artifact: real Parliament data has zero HOC/HOL supplier ID overlap (see the diagnostic finding above). Fixing it would mean regenerating HOC/HOL dummy supplier IDs with non-overlapping ranges, which ripples into every other tab that reads the same supplier master files (Suppliers, AP, PO, Assets) — left unfixed pending a decision on whether that's worth doing, since the real-data behaviour is already confirmed correct.

### Tab-scoped mode loads its cross-domain dependencies too (unlike PO)
`python run_dashboard.py atamis` also loads `supplier_master` (`asuheader`), `po_header`/`po_detail` (`apoheader`/`apodetail`), and — since the `UNIT4_COMMIT_NOT_IN_CONTRACTS` HOL-source fix above — `gl_dimension_values` (`agldimvalue`), alongside the four Atamis files — added explicitly in `load_data()`'s `names_to_load` for `tab == 'atamis'`. Unlike PO (where cross-domain checks are a minority of the suite and the tab-scoped gap is left as a documented limitation), Atamis's cross-system checks are the domain's main value, so `atamis` tab-scoped mode pulls in what it needs to reconcile against rather than leaving everything as `Unknown`. Verified: `run_dq_analysis(frames, tab='atamis')` in tab-scoped mode produces identical check totals/failures to the full dashboard.
...
- 24 checks across all four files (`dashboard/core/rules/atamis_rules.py`, scopes 30–33) — see the Atamis Domain section above for the full design (house derivation, join keys, the 7 cross-system checks with custom modal enrichment)
- **Contract Reference (Atamis) ↔ Contract Id (Unit4 Commitments) confirmed as a direct join** (August 2026, corrected after initial build assumed they were unrelated schemes) — added `ATAMIS_CONTRACT_NOT_IN_COMMITMENTS`, `UNIT4_COMMIT_NOT_IN_CONTRACTS`, and `ATAMIS_CONTRACT_VALUE_MISMATCH`, plus a second overlap bar in the tab's Cross-System Reconciliation section
- **`ATAMIS_COMMIT_*`/`ATAMIS_SPEND_*` renamed to `UNIT4_COMMIT_*`/`UNIT4_SPEND_*`** (August 2026, per direct request) — those two files are Unit4/Agresso views, not Atamis's own data; the original naming misattributed them in both the check ID and the modal's column-prefix labels. Internal frame keys renamed to match (`atamis_commitments`→`unit4_commitments`, `atamis_spend`→`unit4_spend`) so the fix applies to every check on those tables, not just the ones with custom modal enrichment.
- **DQ scorecard split into three sections, then back to two (August 2026, per direct request each time):** first split into resolved (HOC/HOL), Unresolved Records (`Unknown`), and a Table-Wide Checks section for the 7 Unit4 Commitments/Spend checks whose subject has nothing to do with house — surfaced when a `UNIT4_COMMIT_NOT_IN_CONTRACTS` result ("1 flagged of 5 total") turned out to be a fragment of a much larger real number, plus a genuine bug where a blank Contract Id was being counted as an orphan rather than excluded as incomplete. Once the Joint-resolution lookup was widened to cover every unresolved contract, real rows resolve cleanly to HOC/HOL, and the Table-Wide section was folded back into the normal per-house scorecard — the blank-Contract-Id fix was kept, just moved to an ordinary per-house population branch. See "Table-wide population" above for the full history.
- Tab (`dashboard/tabs/atamis.py`) built with a distinct dark-navy/Parliament-green visual identity: hero, HOC/HOL/Joint organisation split, a flagship Cross-System Reconciliation section (supplier overlap bar + 4 cross-system stat cards), top-15 contracts by value, contract lifecycle, contract financials, then the two-section (resolved + Unresolved Records) DQ scorecard/grid described above
...
- `python run_dashboard.py atamis` (tab-scoped mode) also loads `asuheader`/`apoheader`/`apodetail`/`agldimvalue` so cross-system checks work standalone — unlike PO's own tab-scoped mode, which still has this gap (see Atamis Domain section above)
