...
- Plus synthetic HOL rows appended from `agldimvalue` (`dim_position == '5'`, `dim_description == 'Contract Number'`, `dim_value` → `u4_contract_id`), with every other commitment-specific column left blank (HOL has no equivalent rich data) and tagged `_source = 'gl_dimension_values'`.
