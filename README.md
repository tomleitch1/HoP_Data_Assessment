...
```text
C:\Users\TomLeitch\HoP_Data_Assessment\
...
│   ├── tabs/                  # Individual UI pages (ap, ar, gl, findings, explorer, etc.)
...
├── analysis/                  # Analytical notebooks/files
...
1. Place `fixed_assets.csv` in the `data/` folder.
2. Open `dashboard/data_engine.py` and locate the `file_map` inside `load_data()`.
3. Add the mapping:
   ```python
